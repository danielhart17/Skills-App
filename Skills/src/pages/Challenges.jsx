...
export default function Challenges() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const featured = allChallenges.find(c => c.is_featured) || allChallenges.find(c => c.trainer_id);
  ...
  const getCategories = () => {
    return [...new Set(challenges.map(challenge => challenge.category).filter(Boolean))];
  };

  const filteredChallenges = challenges.filter(challenge => {
    // Exclude the featured challenge from the main list if it exists
    const isFeatured = featuredChallenge && challenge.id === featuredChallenge.id;
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-3xl lg:text-4xl font-bold text-gray-900">Challenge Library</h1>
          </div>
          ...
          {featuredChallenge && featuredTrainer ? (
              ...
              <CardContent className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                <div className="md:col-span-2">
                  <Badge className="bg-white text-blue-600 mb-4 uppercase font-semibold">
                    {getCategoryIcon(featuredChallenge.category)} {featuredChallenge.category}
                  </Badge>
                  <h3 className="text-3xl font-bold mb-2">{featuredChallenge.title}</h3>
                  <p className="text-blue-100 mb-6 text-lg">{featuredChallenge.description}</p>
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mb-6">
                    <div className="flex items-center gap-2">
                      <Clock className="w-5 h-5" />
                      <span className="font-semibold">{featuredChallenge.duration_minutes || 20} min</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Trophy className="w-5 h-5" />
                      <span className="font-semibold">+{featuredChallenge.xp_reward} XP</span>
                    </div>
                    <Badge variant="outline" className="bg-transparent border-white text-white uppercase font-semibold">
                      {featuredChallenge.difficulty}
                    </Badge>
                  </div>
                  <Button size="lg" className="bg-white text-blue-600 hover:bg-gray-100 font-bold shadow-lg px-8 py-3">
                ...
                >
                  <p className="text-sm font-semibold uppercase tracking-wider text-blue-200 mb-4">From Trainer</p>
                  <Avatar className="w-24 h-24 mb-4 border-4 border-white/50 group-hover:border-white transition-all shadow-lg">
                    <AvatarImage src={featuredTrainer.profile_image} alt={`${featuredTrainer.name}'s profile`} />
                    <AvatarFallback className="text-blue-600 font-bold text-2xl bg-white">
                      {featuredTrainer.name?.split(' ').map(n => n[0]).join('')}
                    </AvatarFallback>
                  </Avatar>
                  <h4 className="text-xl font-bold mb-1">{featuredTrainer.name}</h4>
                  <div className="flex items-center justify-center gap-1 text-blue-200 mb-2">
                    {featuredTrainer.verified && <CheckCircle className="w-4 h-4 text-green-300"/>}
            ...
            <Card className="border-0 shadow-xl bg-gradient-to-br from-gray-100 to-gray-200">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">No Featured Challenge This Week</h3>
                <p className="text-gray-500">Check back soon for a new trainer challenge!</p>
              </CardContent>
            </Card>
          )}
        </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-red-600 rounded-2xl p-8 text-white text-center shadow-xl">
          ...
          <p className="text-orange-100 mb-6 max-w-md mx-auto">
            Track your makes and misses in real-time with our interactive court tracker
          </p>
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              ...
              size="sm"
              className={selectedCategory === "all" ? "bg-gradient-to-r from-yellow-500 to-orange-600" : ""}
            >
              All
            </Button>
            {getCategories().map((category) => (
              <Button
                ...
                size="sm"
                className={selectedCategory === category ? "bg-gradient-to-r from-yellow-500 to-orange-600" : ""}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            {['all', 'beginner', 'intermediate', 'advanced'].map((difficulty) => (
              <Button
                ...
                size="sm"
                className={selectedDifficulty === difficulty ? "bg-gradient-to-r from-yellow-500 to-orange-600" : ""}
              >
                {difficulty}
              </Button>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredChallenges.map((challenge) => (
            <Card key={challenge.id} className="group hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-white overflow-hidden">
              ...
              <CardContent className="pt-0">
                <div className="space-y-4">
                  ...
                  {challenge.equipment_needed && challenge.equipment_needed.length > 0 && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">Equipment needed:</p>
                      <div className="flex flex-wrap gap-1">
                        {challenge.equipment_needed.map((equipment, index) => (
                  ...
                  <Button className="w-full bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-white shadow-lg hover:shadow-xl transition-all duration-200">
        ...
        {filteredChallenges.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6">Try adjusting your filters or check back later for new challenges</p>
            <Button 
              onClick={() => {
