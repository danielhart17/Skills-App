...
import { ArrowLeft, CheckCircle, Clock, CreditCard, DollarSign } from "lucide-react";
...
const BookingStep = ({ number, title, children, isActive }) => (
  <div className={`transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-40'}`}>
    <div className="flex items-center gap-3 mb-4">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${isActive ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
        {number}
      </div>
...
export default function BookingPage() {
  ...
  const loadBookedTimes = async (trainerId, date) => {
    ...
    const bookings = await Booking.filter({ 
      ...
      booking_datetime: { $gte: startOfDay.toISOString(), $lte: endOfDay.toISOString() }
    });
    ...
    for (let hour = 9; hour < 17; hour++) {
      ...
      for (let minute = 0; minute < 60; minute += 30) { 
        ...
        const isBooked = bookedTimes.some(bookedTime => {
          ...
          const bookedEndTime = add(bookedTime, { minutes: selectedService.duration_minutes }); // Use selectedService.duration_minutes for consistency
        ...
        if (slotTime > thirtyMinutesFromNow && !isBooked) { // Ensure future time and not booked
          slots.push(slotTime);
        }
      }
    }
      ...
      <div className="p-6 lg:p-8 flex items-center justify-center min-h-[calc(100vh-100px)]">
        <Card className="max-w-md w-full text-center p-8 shadow-2xl">
          ...
          <div className="text-left bg-gray-50 p-4 rounded-lg mb-6">
            ...
            <p><strong>Date:</strong> {format(selectedTime, 'MMMM d, yyyy')}</p>
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <h1 className="text-3xl font-bold mb-2">Book a session with {trainer.name}</h1>
        <p className="text-gray-600 mb-8">Follow the steps below to schedule your training.</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-8">
            <BookingStep number={1} title="Select a Service" isActive={step >= 1}>
              {step >= 1 && (
                <div className="space-y-3">
                  {services.map(service => (
                    <button
                      ...
                      className={`w-full text-left p-4 border rounded-lg transition-all ${selectedService?.id === service.id ? 'border-blue-500 bg-blue-50 shadow-md' : 'hover:border-gray-400'}`}
                    >
            ...
            <BookingStep number={2} title="Choose Date & Time" isActive={step >= 2}>
              {step === 2 && selectedService && (
                <div className="space-y-4">
                  ...
                  {generateTimeSlots().length > 0 ? (
                    <div className="grid grid-cols-3 gap-2 max-h-60 overflow-y-auto pr-2"> {/* Added max-height and overflow for scrollable times */}
                    ...
                    <p className="text-gray-500">No available slots for this date and service. Please choose another date or service.</p>
                  )}
                </div>
              )}
            </BookingStep>
            ...
            <BookingStep number={3} title="Confirm & Pay" isActive={step >= 3}>
              {step === 3 && selectedTime && (
                <div className="space-y-4">
                  <p className="text-gray-700">Add any notes for {trainer.name.split(' ')[0]} (e.g., specific skills you want to work on).</p>
                  <Textarea 
            ...
            <div className="row-start-1 md:col-start-2">
              <Card className="sticky top-8 shadow-xl">
                ...
                <CardContent className="space-y-4">
                  ...
                  {selectedTime && (
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold">{format(selectedTime, 'MMM d, yyyy')}</span>
                      </div>
                      <div className="flex justify-between items-center">
                  ...
                  {step === 3 && (
                    <Button onClick={handleConfirmBooking} size="lg" className="w-full mt-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white">
