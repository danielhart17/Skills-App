...
struct Config {
    ...
    // TODO: Replace with `pk_live_...` before App Store submission.
    // The web app uses Stripe Connect (15% platform fee). iOS booking flow
    // currently skips payment — wire it through `createBookingCheckoutURL`
    // when payments come to iOS.
    ...
    // TODO: Set this to your deployed web app URL. Stripe Checkout requires
    // HTTPS success/cancel URLs, so we land the user on the existing web pages
    // that already handle ?success=true / ?canceled=true.
